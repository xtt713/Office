...
cxy_drawfloor.prototype.drawfloorA = function (no, floorno) {
    ...
    var dx = 160;
    var dyvalue = (that.height - 120) / floorno;
    var dy = no * dyvalue + 40;
    var cdrawv = "M" + dx + "," + dy;
    cdrawv += "L" + (that.width - dx * 0.5) + "," + dy;
    cdrawv += "L" + (that.width - dx) + "," + (dy + dyvalue - 180);
    cdrawv += "L" + (dx * 0.5) + "," + (dy + dyvalue - 180);
    cdrawv += "Z";
            ...
            that.floorRArr[i][0].onclick = function () {
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        getBuildingPercent(result.frameInfo[0].HouseID, result.frameInfo[0].FLOOR);
...
//办公楼人员职称分布情况(柱状图)
function getBuildingPercent(buildingID, roomType) {
    var that = this;
    $.ajax({
        url: './Svr/HandlerOffice.ashx',
            ...
            chart = Highcharts.chart('people_chart2', {
                ...
                series: [{
                    ...
                    point: {
                        events: {
                            click: function (e) {
                                columnChartClick(e, buildingID, roomType);
                            }
                        }
                    }
                }]
...
//办公人员职称统计（点击方法）
function columnChartClick(e, buildingID, roomType) {
    switch (e.point.category) {
        case "省级正职":
            getOfficeRoomHouseClick("1", buildingID,roomType);
            break;
        case "省级副职":
            getOfficeRoomHouseClick("2", buildingID, roomType);
            break;
        case "正厅级":
            getOfficeRoomHouseClick("3", buildingID, roomType);
            break;
        case "副厅级":
            getOfficeRoomHouseClick("4", buildingID, roomType);
            break;
        case "正处级":
            getOfficeRoomHouseClick("5", buildingID, roomType);
            break;
        case "副处级":
            getOfficeRoomHouseClick("6", buildingID, roomType);
            break;
        case "处级以下":
            getOfficeRoomHouseClick("7", buildingID, roomType);
            break;
    }

}
//办公人员职称统计（点击方法,改变对应房间背景）
function getOfficeRoomHouseClick(titleID, buildingID, roomType) {
    $.ajax({
        url: './Svr/HandlerOffice.ashx',
